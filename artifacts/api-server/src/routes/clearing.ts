...
router.get("/", async (_req, res) => {
  try {
    ...
    return res.status(500).json({ error: "Failed to fetch clearing batches" });
  }
});
...
router.post("/batches", async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ error: "Failed to create clearing batch" });
  }
});
...
router.post("/batches/:batchId/entries", async (req, res) => {
  ...
  try {
    ...
    return res.status(201).json({ entryId, batchId });
  } catch (err) {
    return res.status(500).json({ error: "Failed to add clearing entry" });
  }
});
...
router.post("/batches/:batchId/submit", async (req, res) => {
  try {
    ...
    return res.json({ submitted: true, batchId: req.params.batchId, status: "submitted" });
  } catch (err) {
    return res.status(500).json({ error: "Failed to submit batch" });
  }
});
...
router.post("/batches/:batchId/settle", async (req, res) => {
  try {
    ...
    return res.json({ settled: true, batchId: req.params.batchId, status: "settled" });
  } catch (err) {
    return res.status(500).json({ error: "Failed to settle batch" });
  }
});
...
router.post("/batches/:batchId/fail", async (req, res) => {
  ...
  try {
    ...
    return res.json({ failed: true, batchId: req.params.batchId, status: "failed" });
  } catch (err) {
    return res.status(500).json({ error: "Failed to mark batch failed" });
  }
});
...
router.get("/stats", async (_req, res) => {
  try {
    ...
    return res.json({ stats, statuses: ["pending", "submitted", "clearing", "settled", "failed"] });
  } catch (err) {
    return res.status(500).json({ error: "Failed to fetch clearing stats" });
  }
});
