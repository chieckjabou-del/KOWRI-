...
## P0 Financial Control Gate (13 septembre 2026)

Rapport complet : `AKWE_P0_FINANCIAL_CONTROL_GATE.md` (16 sections, matrice de contrôles, verdict). Quatre bloqueurs traités à la cause, chacun avec sa preuve exécutable en CI.

- **Cash-in sous maker-checker** (`lib/cashIn.ts`, `routes/cashIn.ts`, migration `0004`) : tables `cash_in_requests` / `cash_in_decisions`, permission `ledger.approve`, initiateur ≠ approbateur ≠ second approbateur imposé par PostgreSQL, exécution atomique avec le dépôt, limites (par opération, par opérateur, par bénéficiaire, plateforme, seuil de seconde signature, expiration) proposées et configurables, clé partagée refusée, écran « Cash-in » du back-office. **Autorité de dépôt obligatoire** : tout dépôt nomme son autorité, la base refuse une création d'argent sans demande `EXECUTED` qui pointe vers elle (`transactions_deposit_authority`, `ledger_float_debit_authority`). Route `POST /wallets/:id/deposit` retirée (410) ; création d'argent par `POST /creator/communities/:id/earnings` fermée (déclaration sans crédit, réservée au créateur). Suite `test-cashin.mjs` (110 vérifications : identité, quatre yeux, concurrence, références, idempotence, cycle de vie, limites, bénéficiaire, routes alternatives HTTP/SQL/fonction interne, traces).
- **Sauvegarde / restauration prouvées** (`scripts/db-backup.sh`, `scripts/db-restore.sh`, `test-disaster-recovery.mjs`, `docs/DISASTER_RECOVERY.md`) : dump vérifié par checksum et manifeste, restauration dans une base isolée, 17 agrégats identiques, triggers actifs, seconde instance API sur la restauration (sessions, PIN, demande `PENDING` approuvée, clé d'idempotence rejouée), documents KYC chiffrés lisibles avec la clé seulement, rotation de clé (`src/tools/rotateKycKey.ts`), perte de `SIGNING_SECRET` sans perte financière. RPO/RTO proposés, à valider.
- **Corruptions injectées** dans la base restaurée : treize refusées par PostgreSQL, six détectées par la réconciliation (I4, I5, I7, I14, I15, I16), une non détectée et nommée (float agent sans compte ledger). Invariants I10–I16 ajoutés, modèle de conservation explicite (créé − détruit = passifs + trésorerie + frais + FX). **`TRUNCATE` bloqué** sur journal, transactions, demandes, décisions et audit (trouvé pendant la matrice : un `TRUNCATE audit_logs` passait).
- **Matrice opérateurs** (`test-operators.mjs`, 226 vérifications) : 20 actions × 8 appelants, cycle de vie des sessions (rôle, mot de passe, MFA, désactivation, déconnexion, expiration), instance `ADMIN_MFA_REQUIRED=true` (sessions non vérifiées et clé partagée en lecture seule, enrôlement, approbation exigeant le second facteur), balayage de toutes les routes d'écriture avec `auditor` et `support` (aucune 2xx hors famille). Nom de kill switch inconnu → 404 (était 500).
- **Passe de concurrence ciblée** (`test-concurrency.mjs`, 95 vérifications) : 2/5/10/20 sur approbation cash-in, transfert, décaissement, remboursement, pool (invest/redeem), FX, float agent, collecte de tontine, réclamation de solidarité, épargne, liquidité trésorerie ; refus de float typé 409 (était 500).
- Architecture du rail fournisseur documentée sans faux fournisseur (`docs/EXTERNAL_RAIL_ARCHITECTURE.md`). Constat : aucune route de cash-out utilisateur n'existe.

## Launch Readiness Gate (13 septembre 2026)

Rapport : `AKWE_LAUNCH_READINESS_GATE.md`. Décision : **CONDITIONALLY READY — ACTIONS REQUIRED** (neuf conditions C1–C9, chacune avec responsable, preuve et étape). Aucun bloqueur technique prouvé ; le périmètre de lancement est réduit à ce qui est prouvé, tout le reste est désactivé par défaut.

- **Périmètre de lancement centralisé** (`lib/launchScope.ts`, `LAUNCH_MODULES`, défaut production `none`) : neuf modules optionnels (crédit, agents, commissions créateurs, pools, assurance, épargne, tontines, FX, cash-out) refusés à la route (503 `MODULE_NOT_IN_LAUNCH_SCOPE`) **et** au point d'entrée de chaque fonction de service qui déplace de l'argent, schedulers tontine/agents inertes quand OFF ; état public sur `GET /health/launch-scope`.
- **Kill switches financiers** (`cash_in`, `credit`, `agent_operations`, `creator_earnings`, `cash_out`, `external_rails`, `fx`) + **propagation inter-instances** (trouvé pendant le gate : un switch tiré sur une instance ne fermait pas les autres avant redémarrage ; désormais adopté en ≤ 5 s, conservateur `FORCED_OFF`, levée manuelle propagée). Tir, verrou et levée alertés et audités.
- **Gate de configuration production** (`secretsCheck.ts`) : refus de démarrer sans `LAUNCH_MODULES`, sans les sept limites `CASH_IN_*` explicites, sans `ALERT_WEBHOOK_URL`, avec des limites non emboîtées, un seuil de seconde signature au-dessus du maximum ou un module mal orthographié. Nouvelle limite `CASH_IN_DAILY_COUNT_PER_BENEFICIARY`.
- **Alerting sortant** (`lib/alerting.ts`) : JSON signé HMAC-SHA256 (`X-Akwe-Signature`) vers `ALERT_WEBHOOK_URL` ; anomalies de réconciliation (critical), kill switches, cash-in expirés ; `POST /admin/alerts/test`, `GET /admin/alerts/status`.
- **Sécurité client** : statut de transaction affiché dans l'app mobile (pending/failed/reversed jamais en succès) ; l'écran « Envoyer » appelait `POST /transactions/transfer` qui **n'existait pas** — route ajoutée (destinataire par numéro, même `processTransfer`, succès seulement si `completed`).
- `POST /admin/patch-tontines` (réparation de fixtures) répond 404 en production. Routes créateur et diaspora conservent la sémantique 503 des refus de périmètre et de switch.
- Runbooks : `docs/RECONCILIATION_RUNBOOK.md` (fréquence, propriétaire à nommer, seuils, escalade, blocage, résolution sans réécriture, rétention 10 ans), `docs/INCIDENT_RESPONSE.md` (DETECT→FREEZE→INVESTIGATE→RECONCILE→RESOLVE→AUDIT ; contre-passation à deux signatures classée P1, procédure transitoire ; pas d'endpoint de suspension client → gel des wallets, P1) ; `.env.launch.example`.
