...
**Bilan** : les six chantiers sont livrés. Restent ouverts, tous consignés au journal : M9 (clés étrangères, soldes par wallet), F2 (lockfile, sur instruction), Content-Security-Policy des front-ends servis, reprise des transferts de float `PENDING`, écran OTP dans l'application mobile, intérêts des prêts (décision produit), cookie `httpOnly` pour le jeton mobile.
