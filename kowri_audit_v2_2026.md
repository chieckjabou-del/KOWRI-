...
État : chantier A **livré le 13 septembre 2026** (voir le journal dans `kowri_audit_gaps_2026.md` ; suite `test-gating.mjs`, 336 routes, 0 ouverte). C1, C2, C3, E1, F5 et M1 sont clos. Chantier B **livré le 13 septembre 2026** : E2 et E3 clos (trésorerie plateforme, décaissement et remboursement par transferts réels, cloisonnement). Constat nouveau consigné au journal : les intérêts des prêts ne sont jamais perçus (décision produit). Chantier C **livré le 13 septembre 2026** : E4, E5, E6, E7 et M2 clos (plafonds par devise, paiement de tontine sur l'encaissé, float atomique, idempotence agents par middleware, parts de pool à la valeur courante). Chantier D **livré le 13 septembre 2026** : M3, M4, M6 et M10 clos (CORS en liste blanche, en-têtes de sécurité, corps borné, arrêt gracieux, verrou inter-instances, TLS PostgreSQL, 404 restreints aux entités introuvables). Chantier E **livré le 13 septembre 2026** : M5, M7, M8, M11 et M12 clos (filtre d'entrée, OTP à l'inscription, purge des sessions, documents KYC chiffrés au repos, reprise sur interblocage, permission `users.read`) ; M9 différé avec plan (clés étrangères après détection des orphelins, soldes matérialisés par wallet). Chantier F **livré le 13 septembre 2026** : F1, F3, F4 et F6 clos (CI GitHub Actions sur migrations versionnées, menus filtrés par permission, jeton mobile borné et révoqué au logout, Node 24 en CI) ; F2 (lockfile) laissé de côté sur instruction, documenté.

**Bilan** : les six chantiers sont livrés ; un audit adversarial d'infrastructure financière a suivi (`AKWE_FINANCIAL_INFRASTRUCTURE_AUDIT.md`), qui a fermé la reprise des transferts de float `PENDING` et ajouté le second facteur opérateur, les invariants de grand livre en base et la réconciliation financière. Restent ouverts, tous consignés au journal : M9 (clés étrangères, soldes par wallet), F2 (lockfile, sur instruction), Content-Security-Policy des front-ends servis, reprise des transferts de float `PENDING`, écran OTP dans l'application mobile, intérêts des prêts (décision produit), cookie `httpOnly` pour le jeton mobile.
